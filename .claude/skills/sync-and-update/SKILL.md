...
## Step 7: Regenerate Interactive HTML Directory

After all README.md and CHANGELOG.md edits are complete, regenerate the interactive HTML directory so it stays in sync:

```bash
python3 scripts/generate-html.py
```

This script reads `README.md` and writes `legal-tech-directory.html`. It is idempotent and safe to run unconditionally. If it fails for any reason, print the error and continue — do NOT abort the commit.

---

## Step 8: Commit and Push

After all edits are complete (README.md from Steps 4–5, CHANGELOG.md from Step 6, and legal-tech-directory.html from Step 7), run the following shell commands:

```bash
# Stage all three files
git add README.md CHANGELOG.md legal-tech-directory.html

# Check if there are staged changes
git diff --cached --quiet && echo "NO_CHANGES" || echo "HAS_CHANGES"
```

**If NO_CHANGES:** Skip commit and push. Go directly to Step 9 and report that no new entries were found.
...
# Count staleness formatting changes
STALE_CHANGES=$(git diff --cached README.md | grep -c 'Stale' || true)

# Commit with descriptive message
git commit -m "chore: add ${ENTRY_COUNT} new entries via sync-and-update, mark ${STALE_CHANGES} stale

Automated update via sync-and-update skill (search + topic crawl + link rot check).
Includes regenerated legal-tech-directory.html.
...
- Do NOT modify any file other than README.md, CHANGELOG.md, and legal-tech-directory.html (which is auto-regenerated by scripts/generate-html.py)
